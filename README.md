...
| **CA_CN**                 | `BigCorp Inc Root CA`| Common name for the Certificate Authority (CA). If not set, constructed as `ORG_NAME` Root CA. |
| **CA_ORG**                | `BigCorp Inc`         | Organization name for the CA. If not set, equals `ORG_NAME`.            |
| **CA_DAYS**               | `1825`                | Expiration days for the CA.                                                 |
| **SRV_CN**                | `BigCorp Inc Server CA` | Common name for the server certificate. If not set, constructed as `ORG_NAME` Server CA. |
| **SRV_ORG**               | `BigCorp Inc`         | Organization name for the server certificate. If not set, equals `ORG_NAME`. |
...
| **DEFAULT_DOMAIN**        | `vpn.bigcorp.com`     | Default domain advertised to clients. Multiple domains can be space-separated. If not set, equals `HOST_NAME`. |
