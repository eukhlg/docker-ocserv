...
RUN apk add --no-cache \
        bash curl g++ gnutls-dev gpgme libev-dev \
...
RUN apk add --no-cache \
        bash gnutls-utils openssl iptables libnl3 readline \
        libseccomp-dev lz4-dev gettext-envsubst
...
COPY --from=builder /tmp/ocserv-default.conf /tmp/ocserv-default.conf
...
RUN sed \
    ...
    -e '/^[[:space:]]*#/d; /^[[:space:]]*$/d' /tmp/ocserv-default.conf > /etc/ocserv/ocserv.conf
